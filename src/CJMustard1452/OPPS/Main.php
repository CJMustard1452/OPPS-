			...
			if($event->getAction() === PlayerInteractEvent::RIGHT_CLICK_AIR){
				...
				if(str_contains(implode($event->getPlayer()->getInventory()->getItemInHand()->getLore()), "Glock")){
					if($event->getPlayer()->hasPermission("opps.admin")){
						...
						$nbt = Entity::createBaseNBT($event->getPlayer()->add($event->getPlayer()->getDirectionVector()->multiply(1.5))->add(0, 2, 0), $event->getPlayer()->getDirectionVector()->multiply(10), $event->getPlayer()->getYaw(), $event->getPlayer()->getPitch());
						$entity = Entity::createEntity("Egg", $event->getPlayer()->getLevel(), $nbt);
						$entity->spawnTo($event->getPlayer());
						$entity->setNameTag($event->getPlayer()->getName());
					}else{
